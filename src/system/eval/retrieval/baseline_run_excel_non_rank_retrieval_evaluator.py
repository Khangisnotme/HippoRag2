"""
This script evaluates retrieval results from an Excel file and generates evaluation metrics.

Input:
    - Excel file (main_vimqa_dev_300lines.xlsx) containing:
        - 'baseline_result_retrieve_raw' sheet with columns:
            - 'supporting_facts': List of tuples containing relevant document IDs
            - 'title': List of retrieved document IDs
    - Location: src/system/eval/input/main_vimqa_dev_300lines.xlsx

Output:
    - Excel file (main_vimqa_dev_300lines_evaluated.xlsx) containing:
        - Original columns plus evaluation metrics:
            - Confusion Matrix Metrics:
                - true_positive: Number of correctly retrieved relevant documents
                - false_positive: Number of incorrectly retrieved irrelevant documents
                - false_negative: Number of missed relevant documents
                - true_negative: Number of correctly not retrieved irrelevant documents
                - total_corpus: Total number of unique documents in corpus
            - Hit Rate:
                - hit_rate: Proportion of queries with at least one relevant document
            - Main Evaluation Metrics:
                - accuracy: Overall accuracy of retrieval
                - precision: Precision at k (k=5)
                - recall_at_k: Recall at k (k=5)
                - f1_score: F1 score at k (k=5)
    - Location: src/system/eval/retrieval/outputs/main_vimqa_dev_300lines_evaluated.xlsx
    - Last row contains average metrics across all queries

Note: The script uses k=5 for evaluation metrics calculation.
"""
...
def main():
    ...
    output_file = script_dir / "outputs" / "main_vimqa_dev_300lines_baseline_non_rank_retrieval_evaluator.xlsx"
    ...
    df = pd.read_excel(input_file, sheet_name="baseline_result_retrieve_raw")
